...
@endsection
@section('page-icon')
    {{ asset('assets/assestsnew/project_task.svg') }}
@endsection
@section('action-btn')

    <div class="float-end">
        @if($view == 'grid')
            <a href="{{ route('organization.taskBoard.view', 'list') }}" class="btn btn-primary btn-sm"
               data-bs-toggle="tooltip"
               title="{{__('List View')}}">
                <span class="btn-inner--text"><i class="ti ti-list"></i></span>
            </a>
        @else
            <a href="{{ route('organization.taskBoard.view', 'grid') }}" class="btn btn-primary btn-sm"
               data-bs-toggle="tooltip"
               title="{{__('Card View')}}">
...
@section('content')
    ...
    <div class="row task-product-wrapper mt-5">
        @if(count($tasks) > 0)
            @foreach($tasks as $task)
                <div class="col-xl-3 col-lg-4 col-md-4 col-sm-6 d-flex flex-column">
                    <div class="card card-progress task-product-card border shadow-none h-100" id="{{$task->id}}"
                        ...
                        <div class="card-body">
                            ...
                            <a class="h6 task-name-break"
                               href="{{ route('organization.projects.tasks.index',!empty($task->project)?$task->project->id:'') }}">{{ $task->name }}</a>
                                    ...
                                    <div class="actions d-flex justify-content-between mt-2 mb-2">
                                        @if(count($task->taskFiles) > 0)
                                            <div class="action-item mr-2" data-bs-toggle="tooltip"
                                                 title="{{ __('Attachment') }}"><i
                                                    class="ti ti-paperclip mr-2"></i>{{ count($task->taskFiles) }}</div>
                                        @endif
                                        @if(count($task->comments) > 0)
                                            <div class="action-item mr-2" data-bs-toggle="tooltip"
                                                 title="{{ __('Comment') }}"
                                                 data-original-title="{{ __('Comment') }}"><i
                                        ...
                                        @if($task->checklist->count() > 0)
                                            <div class="action-item mr-2" data-bs-toggle="tooltip"
                                                 title="{{ __('Checklist') }}"
                                                 data-original-title="{{ __('Checklist') }}"><i
                                                    class="ti ti-list-check mr-2"></i>{{ $task->countTaskChecklist() }}
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-6">@if(!empty($task->end_date) && $task->end_date != '0000-00-00')
                                        <small
...
@push('script-page')
    <script>
        ...
        function ajaxFilterTaskView(task_sort, keyword = '', status = '') {
            ...
            $.ajax({
                url: '{{ route('organization.project.taskboard.view') }}',
                data: data,
