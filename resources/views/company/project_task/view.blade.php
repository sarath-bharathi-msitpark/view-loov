<div class="modal-body task-id p-0" id="{{$task->id}}">
    <div class="card">
        <div class="card-body">
                                ...
                                <div class="progress mb-0">
                                    <div id="progress-result" class="tab-pane tab-example-result fade show active"
                                        ...
                                        <input type="range" class="task_progress custom-range"
                                               ...
                                               data-url="{{ route('organization.change.progress',[$task->project_id,$task->id]) }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="px-3 py-2 row align-items-center">
                            <div class="col">
                                <div class="form-check form-check-inline">
                                    <input type="checkbox" class="form-check-input" id="check-item-{{ $checklist->id }}"
                                           @if($checklist->status) checked
                                           @endif data-url="{{route('organization.checklist.update',[$task->project_id,$checklist->id])}}">
                            ...
                            <div class="col-auto">
                                <div class="action-btn">
                                    <a href="#" class="mx-1 btn btn-sm  align-items-center delete-checklist bg-danger"
                                    ...
                                    <div class="col-auto actions">
                                        <div class="action-btn">
                                            <a href="{{asset(Storage::url('tasks/'.$file->file))}}" download
                                               class="mx-1 btn btn-sm  align-items-center  bg-secondary" role="button"
                                        ...
                                        @auth('web')
                                            <div class="action-btn">
                                                <a href="#"
                                                   class="mx-1 btn btn-sm  align-items-center delete-comment-file  bg-danger"
                        ...
                        <div class="row align-items-center">
                            <div class="col-auto">
                                <a href="#" class="avatar avatar_foredittask avatar-sm ms-2">
                                    <img data-toggle="tooltip" data-original-title="{{(!empty($user)?$user->name:'')}}"
    ...
    <div class="card">
                ...
                @if($plan->chatgpt == 1)
                    <div class="col-6 text-end">
                        <a href="#" data-size="md" class="btn btn-primary btn-icon btn-sm mb-3 me-2"
                           ...
                           data-url="{{ route('organization.grammar',['grammar']) }}"
                        ...
                        <div class="row align-items-start">
                            ...
                            <div class="col-auto">
                                <div class="action-btn me-2">
                                    <a href="#" class="mx-3 btn btn-sm  align-items-center delete-comment bg-danger"
        ...
        <div class="card-footer">
            ...
            <div class="col-12 d-flex align-items-center">
                <div class="avatar avatar_foredittask me-3">
                    <img data-original-title="{{(!empty(\Auth::user()) ? \Auth::user()->name:'')}}"
                         @if(\Auth::user()->avatar) src="{{asset('/storage/uploads/avatar/'.\Auth::user()->avatar)}}"
                         @else src="{{asset('/storage/uploads/avatar/avatar.png')}}"
                         @endif title="{{ Auth::user()->name }}"
                         class="wid-40 rounded border-2 border border-primary ml-3">
                </div>
                <div class="form-group mb-0 form-send w-100">
                    <form method="post" class="card-comment-box" id="form-comment"
                        ...
                        <textarea rows="1" class="form-control grammer_textarea mt-0" name="comment" data-toggle="autosize"
                                  placeholder="{{__('Add a comment...')}}"></textarea>
                    </form>
                </div>

                <button id="comment_submit" class="btn btn-send"><i class="text-primary ti ti-brand-telegram"></i>
                </button>
...
@push('script-page')
    ...
    <!--                    $.ajax({-->
    <!--                        url: '{{ route('organization.update.task.priority.color') }}',-->
    <!--                        method: 'PATCH',-->
    <!--                        data: {-->
    <!--                            'task_id': task_id,-->
    <!--                            'color': color,-->
