                ...
                <div class="float-left">
                    <h5 class="week-date">
                        ...
                        @elseif($lastWeek == $key)
                            {{__('Last week')}}
                        @else
                            {{date('M d',strtotime($date['start_date']. ' +1 day'))}}
                            - {{date('M d',strtotime($date['end_date']. ' +1 day'))}}
                        @endif
                    </h5>
                </div>
                                        ...
                                        <div class="row acc-sub-menu-div">
                                            ...
                                            <div class="col-1">
                                                <img alt="Image placeholder"
                                                     ...
                                                     data-id="{{$t['id']}}" id="track-images-{{$t['id']}}">
                                                <i data-id="{{$t['id']}}" data-is_billable="{{$t['is_billable']}}"
                                                   data-toggle="tooltip"
                                                   data-original-title="{{$t['is_billable'] ==1? __('Click to Mark Non-Billable'):__('Click to Mark Billable')}}"
                                                   class="change_billable ti ti-dollar-sign {{$t['is_billable'] ==1?'doller-billable':'doller-non-billable'}}"></i>
                                                <i class="ti ti-times text-danger mx-2 pointer remove-track "
                                                   data-toggle="tooltip" data-original-title="{{__('Delete')}}"
                                                   data-id="{{$t['id']}}" data-url=""></i>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
