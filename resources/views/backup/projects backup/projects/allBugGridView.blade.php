...
@section('action-btn')
    <div class="float-end">
        @if($view == 'grid')
            <a href="{{ route('organization.bugs.view', 'list') }}" class="btn btn-primary btn-sm me-1"
               data-bs-toggle="tooltip"
               title="{{__('List View')}}">
                <span class="btn-inner--text"><i class="ti ti-list"></i></span>
            </a>
        @else
            <a href="{{ route('organization.bugs.view', 'grid') }}" class="btn btn-primary btn-sm me-1">
                <span class="btn-inner--text"><i class="ti ti-table"></i>{{__('Card View')}}</span>
            </a>
        @endif
        <a href="{{ route('organization.projects.index') }}" class="btn bg-brown-subtitle text-white btn-sm me-1"
                ...
                @if(count($bugs) > 0)
                    @foreach($bugs as $bug)
                        <div class="col-md-4 col-xxl-3 col-sm-6 d-flex">
                            <div class="card card-progress" id="{{$bug->id}}"
                                 style="{{ !empty($bug->priority_color) ? 'border-left: 2px solid '.$bug->priority_color.' !important' :'' }};">
                                <div class="card-body">
                                    <div class="h-100 d-flex flex-column">
                                        <div class="mb-2 d-flex align-items-center justify-content-between">
                                              <span>
                                                <a href="{{ route('organization.task.bug.kanban',$bug->project_id) }}"
                                                   class="text-body h6">{{$bug->title}}</a>
                                              </span>
                                            @if($bug->priority =='low')
                                                <span
                                                    class="status_badge badge bg-success p-2 px-3 rounded">{{ ucfirst($bug->priority) }}</span>
                                            @elseif($bug->priority =='medium')
                                                <span
                                                    class="status_badge badge bg-warning p-2 px-3 rounded">{{ ucfirst($bug->priority) }}</span>
                                            @elseif($bug->priority =='high')
                                                <span
                                                    class="status_badge badge bg-danger p-2 px-3 rounded">{{ ucfirst($bug->priority) }}</span>
                                            @endif
                                        </div>
                                        <div class="mb-3 d-flex align-items-center justify-content-between gap-2">
                                            <p class="mb-0">
                                                <span
                                                    class="mb-2 d-inline-block text-sm">{{(!empty($bug->description)) ? $bug->description : '-'}}</span>
                                            </p>
                                            <p class="mb-0">
