<table class="table mb-0">
    ...
    @if(isset($allProjects) && $allProjects == true)
        @foreach ($timesheetArray as $key => $timesheet)
            <tr>
                <td class="project-name" data-bs-toggle="tooltip"
                    title="{{__('Project')}}">{{ $timesheet['project_name'] }}</td>
            </tr>
            @foreach ($timesheet['taskArray'] as $key => $taskTimesheet)
                @foreach ($taskTimesheet['dateArray'] as $dateTimeArray)
                    <tr class="timesheet-user">
                        <td class="task-name" data-bs-toggle="tooltip"
                            title="{{__('Task')}}">{{ $taskTimesheet['task_name'] }}</td>
                        @foreach ($dateTimeArray['week'] as $dateSubArray)
                            <td>
                                <input
                                    class="form-control {{ $dateSubArray['time'] != '00:00' ? 'border-dark' : '-' }} wid-120 task-time day-time"
                                    ...
                                    type="text"
                                    value="{{ $dateSubArray['time'] != '00:00' ? $dateSubArray['time'] : '00:00' }}">
                            </td>
                        @endforeach
                        <td class="text-center total-task-time day-time">
                            <input class="form-control border-dark wid-120 total-task-time day-time"
                                   type="text"
                                   value="{{$dateTimeArray['totaltime'] != '00:00' ? $dateTimeArray['totaltime'] : '00:00' }}">
                        </td>
                    </tr>
                @endforeach
            @endforeach
        ...
        @foreach ($timesheetArray as $key => $timesheet)
            <tr>
                ...
                @foreach ($timesheet['dateArray'] as $day => $datetime)
                    <td>
                        <input
                            class="form-control {{ $datetime['time'] != '00:00' ? 'border-dark' : '00:00' }} wid-120 task-time day-time1"
                ...
                @endforeach
                <td class="text-center total-task-time day-time1"><input
                        class="form-control border-dark wid-120 task-time day-time1" type="text"
                        value="{{$timesheet['totaltime'] != '00:00' ? $timesheet['totaltime'] : '00:00' }}">
                </td>
            </tr>
        @endforeach
    @endif
    ...
    <tfooter>
        <tr class="bg-primary">
            ...
            @foreach ($totalDateTimes as $key => $totaldatetime)
                <td class="total-date-time"><input
                        class="form-control bg-transparent {{ $totaldatetime != '00:00' ? 'border-dark' : 'border-dark' }}  wid-120"
                        type="text" value="{{ $totaldatetime != '00:00' ? $totaldatetime : '00:00' }}"></td>
            @endforeach
            <td class="text-center total-value1">
                <input
                    class="form-control bg-transparent {{ $calculatedtotaltaskdatetime != '00:00' ? 'border-dark' : 'border-dark' }} wid-120"
                    type="text"
                    value="{{ $calculatedtotaltaskdatetime != '00:00' ? $calculatedtotaltaskdatetime : '00:00' }}">
            </td>
        </tr>
    </tfooter>
</table>
...
<div class="text-center d-flex align-items-center justify-content-center mt-4 mb-5 timelogged">
    ...
    <span
        class="p-2  f-w-900 rounded  bg-primary d-inline-block border border-dark">{{ $calculatedtotaltaskdatetime . __(' Hours') }}</span>
</div>
