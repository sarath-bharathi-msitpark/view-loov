...
@section('action-btn')
    <div class="float-end">
        <div class="btn-group mr-2" id="change_view" role="group">
            <a href="{{route('organization.projects.gantt',[$project->id,'Quarter Day'])}}"
               class="btn btn-primary @if($duration == 'Quarter Day')active @endif"
               data-value="Quarter Day">{{__('Quarter Day')}}</a>
            <a href="{{route('organization.projects.gantt',[$project->id,'Half Day'])}}"
               class="btn btn-primary @if($duration == 'Half Day')active @endif"
               data-value="Half Day">{{__('Half Day')}}</a>
            <a href="{{route('organization.projects.gantt',[$project->id,'Day'])}}"
               class="btn btn-primary @if($duration == 'Day')active @endif" data-value="Day">{{__('Day')}}</a>
            <a href="{{route('organization.projects.gantt',[$project->id,'Week'])}}"
               class="btn btn-primary @if($duration == 'Week')active @endif" data-value="Week">{{__('Week')}}</a>
            <a href="{{route('organization.projects.gantt',[$project->id,'Month'])}}"
               class="btn btn-primary @if($duration == 'Month')active @endif" data-value="Month">{{__('Month')}}</a>
        </div>
        <a href="{{ route('organization.projects.show',$project->id) }}" class="btn btn-primary "
            ...
            <div class="card card-stats border-0">
                ...
                @else
                    ...
                    <div class="page-search">
                        ...
                        <div class="mt-3">
                            <a class="btn-return-home badge-blue" href="{{route('dashboard')}}"><i
        ...
        <script>
            ...
            var gantt_chart = new Gantt(".gantt-target", tasks, {
                ...
                on_date_change: function (task, start, end) {
                    ...
                    $.ajax({
                        url: "{{route('organization.projects.gantt.post',[$project->id])}}",
                        data: {
