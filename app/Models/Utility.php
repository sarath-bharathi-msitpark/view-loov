...
//                    dd(\Storage::disk(),$path);
    ...
    public static function deleteOldFiles($path, $daysOld = 30)
    {
        try {
            $settings = Utility::settings();
            $deleted = [];

            // Detect and build the correct disk
            $disk = match ($settings['storage_setting']) {
                'wasabi' => \Storage::build([
                    'driver' => 's3',
                    'key' => $settings['wasabi_key'],
                    'secret' => $settings['wasabi_secret'],
                    'region' => $settings['wasabi_region'] ?? 'ap-southeast-1',
                    'bucket' => $settings['wasabi_bucket'],
                    'endpoint' => $settings['wasabi_url'] ?? 'https://s3.ap-southeast-1.wasabisys.com',
                ...
                's3' => \Storage::build([
                    ...
                    'endpoint' => $settings['s3_url'] ?? null,
                ]),
                default => \Storage::disk('local'),
            };

            \Log::info("Checking old files on disk: {$settings['storage_setting']}, path: {$path}");

            $files = $disk->allFiles($path);

            foreach ($files as $file) {
                $lastModified = $disk->lastModified($file);
                $lastModifiedDate = \Carbon\Carbon::createFromTimestamp($lastModified)->timezone(config('app.timezone'));

                if ($lastModifiedDate->lt(now()->subDays($daysOld))) {
                    if ($disk->delete($file)) {
                        $deleted[] = $file;
            ...
            return [
                'success' => 1,
                'deleted' => $deleted,
                'message' => count($deleted) . " file(s) deleted older than {$daysOld} days.",
            ];
        } catch (\Exception $e) {
            \Log::error("Delete old files failed: " . $e->getMessage());
            return [
                'success' => 0,
                'message' => $e->getMessage(),
            ];
