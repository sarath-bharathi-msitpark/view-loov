...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function assignPlan($planID, $company_id = 0)
    {
        ...
        if ($plan) {
            ...
            if ($this->trial_expire_date != null) {
                ...
                $this->trial_plan = null;
            }

            if ($plan->duration == 'month') {
                $this->plan_expire_date = Carbon::now()->addMonths(1)->isoFormat('YYYY-MM-DD');
            } elseif ($plan->duration == '3month') {
                $this->plan_expire_date = Carbon::now()->addMonths(3)->isoFormat('YYYY-MM-DD');
            } elseif ($plan->duration == 'year') {
                $this->plan_expire_date = Carbon::now()->addYears(1)->isoFormat('YYYY-MM-DD');
            } elseif ($plan->duration == 'day') {
                $this->plan_expire_date = Carbon::now()->addDay(1)->isoFormat('YYYY-MM-DD');
            } else {
                $this->plan_expire_date = null;
            }
            $this->save();

            $this->syncPermissions($plan->permissions->pluck('id')->toArray());
            ...
            \Log::info('Plan updated', [
                ...
                'old_plan_id' => $oldPlan,
                'new_plan_id' => $plan->id,
                'expire_date' => $this->plan_expire_date,
    ...
    public function assignTrialPlan($planID, $company_id = 0)
    {
        $plan = Plan::find($planID);
        if ($plan) {

            $trialDays = $plan->trial_days ?? 7;
            $newDate = date('Y-m-d', strtotime(date('Y-m-d') . ' + ' . $trialDays . ' days'));
            $this->plan_expire_date = $newDate;
            $this->plan = $plan->id;
            $this->trial_expire_date = $newDate;
            $this->trial_plan = $plan->id;
            $this->save();
    ...
    {
        return $this->currentPlan?->max_users ?? 0;
    }
    
    public function getOwnerMaxUsersAttribute()
    {
        $owner = ($this->type == 'company' || $this->type == 'super admin')
            ? $this
            : User::find($this->created_by);
    
        return $owner?->max_users ?? 0;
    }
