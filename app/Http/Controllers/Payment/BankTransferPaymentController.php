...
class BankTransferPaymentController extends Controller
{
    ...
    public function planPayWithBank(Request $request)
    {
        ...
        if($plan)
        {
            $price = ($plan->getTotalPlanAmount($plan->id));
            if(isset($request->coupon) && !empty($request->coupon))
            {
                ...
                if(!empty($coupons))
                {
                    ...
                    $userCoupun     = UserCoupon::where('user', \Auth::user()->id)->where('coupon', $coupons->id)->first();
                    if(!empty ($userCoupun)){
                        return redirect()->back()->with('error', __('This coupon code has already been applied.'));
                    }
                    $discount_value = (($plan->getTotalPlanAmount($plan->id)) / 100) * $coupons->discount;
                    $price          = ($plan->getTotalPlanAmount($plan->id)) - $discount_value;
                ...
                if(!empty($request->payment_receipt))
                {
                    if($authuser->type == 'company') {
                        $companySlug = Str::slug($authuser->company_name);
                        $dir = "uploads/companies/{$companySlug}/order";
                    } else {
                        ...
                        $dir = "uploads/companies/{$companySlug}/employees/{$nameSlug}/order";
                    }
                    $fileName = time() . "_" . $request->payment_receipt->getClientOriginalName();
                    
                    $uploadResult = Utility::upload_file($request,'payment_receipt',$fileName,$dir,[]);
            ...
            return redirect()->route('general.plans.index')->with('success', __('Plan payment request send successfully'));
    ...
    public function action($id)
    {
        ...
        return view('shared.order.action', compact('order','admin_payment_setting'));
    }
    ...
    public function changeStatus(Request $request , $order_id)
    {
        ...
        return redirect()->route('general.order.index')->with('success', __('Plan payment status updated successfully.'));
    }
