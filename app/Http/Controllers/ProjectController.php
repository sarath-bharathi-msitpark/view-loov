...
class ProjectController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Email notification (to first assigned user if exists)
            if (!empty($request->users)) {
                $firstUser = User::find($request->users[0]);
                if ($firstUser) {
                    $setting = Utility::settings(\Auth::user()->creatorId());

                    if (!empty($setting['new_project']) && $setting['new_project'] == 1) {
                        $projectArr = [
                            'project_user' => $firstUser->name,
                            'project_name' => $project->project_name,
                        ];
                        Utility::sendEmailTemplate('new_project', [$firstUser->id => $firstUser->email], $projectArr);
                    }
                }
            }
            ...
            if ($webhook) {
                ...
                if ($status == false) {
                    DB::commit();
                    return redirect()->back()->with('error', __('Project added successfully, but Webhook call failed.'));
                }
            }
...
//        if (in_array($project->id, $user_projects)) {
        ...
        return view('company.projects.view', compact('project', 'project_data', 'last_task'));
//        } else {
...
//            return view('company.projects.edit', compact('project', 'clients'));
//        } else {
...
//    }
    ...
    public function edit(Project $project)
    {
        ...
        // Assuming you have relations in Project model: teams() & users()
...
//    }
    ...
    public function update(Request $request, Project $project)
    {
        ...
        $project->project_name = $request->project_name;
        $project->start_date = $request->start_date ? date("Y-m-d H:i:s", strtotime($request->start_date)) : null;
        $project->end_date = $request->end_date ? date("Y-m-d H:i:s", strtotime($request->end_date)) : null;
        $project->budget = $request->budget;
        $project->client_id = $request->client;
        $project->description = $request->description;
        $project->status = $request->status;
        ...
        // If your project has many-to-many relation with teams/users:
        ...
        return redirect()->route('projects.index')
            ->with('success', __('Project Updated Successfully') . ((isset($result) && $result != 1) ? '<br> <span class="text-danger">' . $result . '</span>' : ''));
    }
...
//        if (\Auth::user()->can('create milestone')) {
        ...
        return view('company.projects.milestone', compact('project'));
//        } else {
//            return redirect()->back()->with('error', __('Permission Denied.'));
//        }
    }
    ...
    public function milestoneEdit($id)
    {
        ...
        return view('company.projects.milestoneEdit', compact('milestone'));
    }
    ...
    public function milestoneShow($id)
    {
        ...
        return view('company.projects.milestoneShow', compact('milestone'));
    }
    ...
    public function filterProjectView(Request $request)
    {
        ...
        if ($usr->type == 'client') {
            $user_projects = Project::where('client_id', $usr->id)
                ->where('created_by', $usr->creatorId())
                ->pluck('id')
                ->toArray();
        } else {
            $assigned_projects = $usr->projects()->pluck('project_id')->toArray();
            $created_projects  = Project::where('created_by', $usr->creatorId())
                ->pluck('id')
                ->toArray();
        ...
        $allowedViews = ['grid', 'list']; // adjust to your actual view names
        $view = in_array($request->view, $allowedViews) ? $request->view : 'grid';
    
        $sortRaw   = $request->input('sort', 'created_at-desc');
        $sort      = explode('-', $sortRaw);
        $sortField = in_array($sort[0] ?? '', ['created_at', 'project_name']) ? $sort[0] : 'created_at';
        $sortOrder = ($sort[1] ?? 'desc') === 'asc' ? 'asc' : 'desc';
    
        $query = Project::whereIn('id', $user_projects)
        ...
        if (!empty($request->project_ids)) {
            $ids = array_filter(array_map('intval', explode(',', $request->project_ids)));
            if (!empty($ids)) {
        ...
        if (!empty($request->date_from)) {
            $query->whereDate('on_board_date', '>=', $request->date_from);
        }
        if (!empty($request->date_to)) {
            $query->whereDate('on_board_date', '<=', $request->date_to);
        }
    
        $projects  = $query->get();
        $last_task = \App\Models\TaskStage::orderBy('order', 'DESC')
            ->where('created_by', $usr->creatorId())
            ->first();
    
        $returnHTML = view(
            'company.projects.' . $view,
            compact('projects', 'user_projects', 'last_task')
        )->render();
    ...
    public function bug($project_id)
    {
        ...
        if (!empty($project) && $project->created_by == Auth::user()->creatorId()) {
            ...
            return view('company.projects.bug', compact('project', 'bugs'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function bugCreate($project_id)
    {
        ...
        return view('company.projects.bugCreate', compact('status', 'project_id', 'priority', 'users'));
    }
    ...
    public function bugEdit($project_id, $bug_id)
    {
        ...
        return view('company.projects.bugEdit', compact('status', 'project_id', 'priority', 'users', 'bug'));
    ...
    public function bugKanban($project_id)
    {
        ...
        if (!empty($project) && $project->created_by == $user->creatorId()) {
            ...
            return view('company.projects.bugKanban', compact('project', 'bugStatus'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
...
//        dd($request->all());
    ...
    public function bugShow($project_id, $bug_id)
    {
        ...
        return view('company.projects.bugShow', compact('bug'));
    }
    ...
    public function copyproject($id)
    {
        ...
        return view('company.projects.copy', compact('project'));
    }
