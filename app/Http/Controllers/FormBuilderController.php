...
class FormBuilderController extends Controller
{
    public function index()
    {
        ...
        return view('company.form_builder.index', compact('forms'));
    }

    public function create()
    {
        return view('company.form_builder.create');
    }
    ...
    public function show(FormBuilder $formBuilder)
    {
        if ($formBuilder->created_by == \Auth::user()->creatorId()) {
            return view('company.form_builder.show', compact('formBuilder'));
        } else {
    ...
    {
        if ($formBuilder->created_by == Auth::user()->creatorId()) {
            return view('company.form_builder.edit', compact('formBuilder'));
        } else {
            return response()->json(['error' => __('Permission Denied.')], 401);
        }
    }
    ...
    public function fieldCreate($id)
    {
        ...
        if ($formbuilder->created_by == $usr->creatorId()) {
            ...
            return view('company.form_builder.field_create', compact('types', 'formbuilder'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function fieldEdit($id, $field_id)
    {
        ...
        if ($form->created_by == $usr->creatorId()) {
            ...
            if (!empty($form_field)) {
                ...
                return view('company.form_builder.field_edit', compact('form_field', 'types', 'form'));
            } else {
    ...
    public function viewResponse($form_id)
    {
        ...
        if (!empty($form)) {
            if ($form->created_by == \Auth::user()->creatorId()) {
                return view('company.form_builder.response', compact('form'));
            } else {
                return response()->json(['error' => __('Permission Denied . ')], 401);
            }
        }
    ...
    public function responseDetail($response_id)
    {
        ...
        if ($form->created_by == \Auth::user()->creatorId()) {
            ...
            return view('company.form_builder.response_detail', compact('response'));
        } else {
            return response()->json(['error' => __('Permission Denied . ')], 401);
        }
    }
        ...
        if (!empty($code)) {
            ...
            if (!empty($form)) {
                if ($form->is_active == 1) {
                    ...
                    return view('company.form_builder.form_view', compact('objFields', 'code', 'form'));
                } else {
                    return view('company.form_builder.form_view', compact('code', 'form'));
                }
            } else {
    ...
    public function formFieldBind($form_id)
    {
        ...
        if ($usr->type == 'company') {
            ...
            if ($form->created_by == $usr->creatorId()) {
                ...
                return view('company.form_builder.form_field', compact('form', 'types', 'formField', 'users', 'pipelines'));
            } else {
                return redirect()->back()->with('error', __('Permission Denied.'));
            }
        } else {
