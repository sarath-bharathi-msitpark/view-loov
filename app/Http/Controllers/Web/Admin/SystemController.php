...
class SystemController extends Controller
{
    public function index()
    {
        if (\Auth::user()->can('manage system settings')) {
            ...
            return view('admin.settings.index', compact('settings', 'admin_payment_setting', 'file_size'));
        } else {
    ...
    public function saveCompanyPaymentSettings(Request $request)
    {
        ...
        if (isset($request->is_bank_transfer_enabled) && $request->is_bank_transfer_enabled == 'on') {
            ...
            if ($request->file('bank_upi_qr')) {
                $filenameWithExt = $request->file('bank_upi_qr')->getClientOriginalName();
                $filename = pathinfo($filenameWithExt, PATHINFO_FILENAME);
                $extension = $request->file('bank_upi_qr')->getClientOriginalExtension();
                ...
                $bankUpiQrPath = '';
                $path = Utility::upload_file($request, 'bank_upi_qr', $fileNameToStore, $dir, []);
                if ($path['flag'] == 1) {
                    $bankUpiQrPath = $path['url'];
                    $post['bank_upi_qr'] = $bankUpiQrPath;
                } else {
                    return redirect()->back()->with('error', __($path['msg']));
                }
            }
    ...
    public function adminPaymentSettings($request)
    {
        ...
        $post['default_price_per_user'] = $request->default_price_per_user;
     ...
     */
    public function downloadlinkSetting(Request $request)
    {
        ...
        if ($apkUpdated) {
            User::where('type', 'company')->update([
                'is_apk_update_notified' => true
            ]);
        }

        return redirect()->back()->with('success', __('Download url Setting successfully saved.'));
    }
