...
class PlanRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            $plan_requests = PlanRequest::whereHas('user')->orderBy('created_at', 'desc')->get();

            return view('shared.plan_request.index', compact('plan_requests'));
        }
        else
        {
    ...
    */
    ...
    public function userRequestTrail($plan_id)
    {
        $objUser = Auth::user();


        if(empty($objUser->plan) && empty($objUser->plan_expire_date))
        {
            $planID = \Illuminate\Support\Facades\Crypt::decrypt($plan_id);
            $plan = Plan::find($planID);
            if(!empty($planID))
            {
                $objUser->assignTrialPlan($planID);

                return redirect()->back()->with('success', __('Trail plan activated successfully.'));
            }
            else
            {
                return redirect()->back()->with('error', __('Something went wrong.'));
    ...
    */
    public function acceptRequest($id, $response)
    {
        ...
        if(Auth::user()->type == 'super admin')
        {
            ...
            {
                $user = User::where('id', $plan_request->user_id)->firstOrFail();
                if(empty($plan_request)) {
                     return redirect()->back()->with('error', __('Something went wrong.'));
                }
                if($response == 1)
...
//                    $assignPlan = $user->assignPlan($plan_request->plan_id, $plan_request->duration);
                    ...
                    $user->syncPermissions($plan->permissions->pluck('id')->toArray());
    ...
    */
    ...
    public function increaseLicense($plan_id)
    {
        $user = \Auth::user();
    
        if ($user->type !== 'company') {
            return redirect()->back()->with('error', 'Access denied. Only company users can manage user licenses.');
        }
        ...
        if ($user->plan != $plan->id) {
            return redirect()->back()->with('error', 'You are not subscribed to this plan.');
        }
    
        $plan_expiry_date = $user->plan_expire_date;
        $expired = false;
    
        if (!empty($plan_expiry_date)) {
            $today = now()->startOfDay();
            $expiry = \Carbon\Carbon::parse($plan_expiry_date)->startOfDay();
            $expired = $expiry->lessThan($today);
        }
    
        return view('shared.plan.increaseUsers', compact('plan', 'plan_expiry_date', 'expired'));
    }
}
