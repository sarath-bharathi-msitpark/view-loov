...
class StripePaymentController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($objUser->type == 'super admin') {
            $orders = Order::select([
                'orders.*',
                'users.company_name as user_name',
            ...
            return view('shared.order.index', compact('orders', 'userOrders'));
            ...
            return view('shared.order.index', compact('orders'));
        }
    }
    ...
    public function stripe($code, $mode = null)
    {

        $admin_payment_setting = Utility::getAdminPaymentSetting();
        if (isset($admin_payment_setting) && !empty($admin_payment_setting)) {
            if ($admin_payment_setting['is_bank_transfer_enabled'] == 'on' || $admin_payment_setting['is_stripe_enabled'] == 'on' || $admin_payment_setting['is_cashfree_enabled'] == 'on') {
                ...
                $activeUserCount = User::whereHas('roles', function ($query) {
                    ...
                    ->count();
                if ($activeUserCount > $plan->max_users) {
                    $contact_support = Utility::getValByName('contact_support');
                
                    $message = "This plan supports up to {$plan->max_users} active users. "
                             . "You currently have {$activeUserCount}. "
                             . "Please deactivate a few users before switching to this plan.";
                
                    if (!empty($contact_support)) {
                        $message .= " If you need help, please contact support at {$contact_support}.";
                    }
                ...
                if ($mode === 'trial') {
                    if (!empty(\Auth::user()->plan) || !empty(\Auth::user()->plan_expire_date)) {
                        return redirect()->back()->with('error', __('Trial already used.'));
                    }
                    $paymentMode = "trail";
                }
            
                if (!empty(\Auth::user()->stripe_customer_id) && (\Auth::user()->payment_mode == "stripe")) {
                    $newRequest = new Request([
                        'plan_id' => $code,
                    ]);
                    return app(\App\Http\Controllers\Payment\StripeSubscriptionController::class)->createCheckoutSession($newRequest);
                }else if ((\Auth::user()->payment_mode == "cashfree")) {
                    $newRequest = new Request([
                        'plan_id' => $code,
                    ]);
                    return app(\App\Http\Controllers\Payment\CashfreeController::class)->startSubscription($newRequest);
                }
            
                return view('shared.stripe', compact('plan', 'admin_payment_setting', 'paymentMode'));
                
            } else {
                return redirect()->route('general.plans.index')->with('error', __('The admin has not set the payment method.'));
            }
        }
    }
            ...
            try {
                ...
                if ($data['amount_refunded'] == 0 && empty($data['failure_code']) && $data['paid'] == 1 && $data['captured'] == 1) {
                    ...
                    if ($data['status'] == 'succeeded') {
                        ...
                        return redirect()->route('general.plans.index')->with('error', __('Your payment has failed.'));
                    }
                } else {
                    return redirect()->route('general.plans.index')->with('error', __('Transaction has been failed.'));
                }
            } catch (\Exception $e) {
    ...
    public function testindex()
    {
        ...
        return view('shared.stripetest', compact('admin_payment_setting'));
    }
    ...
    public function teststore(\Illuminate\Http\Request $request)
{
    ...
    try {
        // 1) Create / attach a Customer with name + billing address (required for India exports)
        $customer = \Stripe\Customer::create([
            'name'    => $request->input('name'),
            'email'   => $request->input('email'),
            'address' => [
                'line1'       => $request->input('billing_address.line1'),
                'line2'       => $request->input('billing_address.line2'),
        ...
        // OPTIONAL: shipping (required for export of physical goods)
        $shipping = null;
        if ($request->filled('shipping.name') && $request->filled('shipping.address.line1')) {
            $shipping = [
        ...
        // 2) Create PaymentIntent with description (+ shipping if goods)
        $intent = \Stripe\PaymentIntent::create([
            'amount'   => 100 * 100,   // ₹100 in paise
            'currency' => 'inr',
            'customer' => $customer->id,
            'description' => 'Export of software services - Order #1234', // <-- REQUIRED for exports
            'shipping' => $shipping,   // include when shipping physical goods
            'automatic_payment_methods' => [
                'enabled' => true,
                'allow_redirects' => 'never', // card-only, no return_url needed
            ],
        ]);
